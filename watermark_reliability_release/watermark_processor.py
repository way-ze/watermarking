# Wayne: Code has been taken from the github repo below.
# __git
...
from watermark_reliability_release.normalizers import normalization_strategy_lookup
from watermark_reliability_release.alternative_prf_schemes import prf_lookup, seeding_scheme_lookup
...
class WatermarkBase:
    ...
    ### Modified to also output the redlist - Wayne ###
    def _get_greenlist_ids(self, input_ids: torch.LongTensor, get_redlist=False):
        """Seed rng based on local context width and use this information to generate ids on the green list."""
        self._seed_rng(input_ids)
        ...
        if get_redlist:
            ...
            # do sanity check here to make sure that the greenlist and redlist indices are empty
